...
void testAdventurerCard() {
    ...
    for (i = 0; i < beginningGameState.handCount[0]; i++) {
        if (beginningGameState.hand[0][i] == copper || beginningGameState.hand[0][i] == silver || beginningGameState.hand[0][i] == gold) {
            beginningTreasures++;
        }
    }
    ...
    for (i = 0; i < currentGameState.handCount[0]; i++) {
        if (currentGameState.hand[0][i] == copper || currentGameState.hand[0][i] == silver || currentGameState.hand[0][i] == gold) {
            currentTreasures++;
        }
    }
